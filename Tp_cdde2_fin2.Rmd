...
```{r Carga nominación de Barrios de la Ciudad, message=FALSE, warning=FALSE, results = 'hide'}
...
```{r Carga de Barrios Vulnerables, message=FALSE, warning=FALSE, results = 'hide'}
td <- tempdir()
...
```{r Visualizaciones1: Barrios vulnerables de la Ciudad, echo=TRUE}
...
```{r Carga de Mapa base Radio Censales AMBA, include=FALSE, results = 'hide'}
td <- tempdir()
...
```{r Selección de 3 barrios populares y muestra de 8 radiocensales de cada uno, message=FALSE, warning=FALSE, results = 'hide'}
...
```{r Visualizaciónes2:  Muestra de 8 Radiocensales, echo=TRUE}
...
```{r Carga de escuelas, message=FALSE, warning=FALSE, results = 'hide'}
...
```{r Visualizaciones3: Radio de cobertura de escuelas vulnerables , echo=TRUE}
ggplot() +
...
```{r Operaciones espaciales: Bufferrs y conteo de escuelas en zonas vulnerables, results = 'hide' }
...
```{r Graficos 1: Conteo de escuelas por radio de cobertura, echo=TRUE, }
...
```{r Graficos2: Escuelas en radios de cobertura por gestión}
...
```{r Visualizaciones4: Buffers y escuelas de barrios vulnerables}
...
```{r Visualizaciones6: Interactividad}
leaflet() %>%
  addProviderTiles("CartoDB.Positron") %>%
...
```{r Graficos3: Promedio de tiempo de viaje a pie por radiocensa}
...
```{r Operaciones espaciales2: Join Espacial con No espacial: Radio Censales e Ingreso}
...
```{r Operaciones de datos3: Clasificacion por de ingreso en cuartiles}
...
```{r Operaciones espaciales3: Joins espaciales}
...
```{r Carga mapa radiocensales CABA}
#Cargamos un mapa de radiocensales de CABA para que nos oficie de mapa base para la visualización
radio_bsas <-  st_read(url_radiocensalcaba)
st_crs(radio_bsas)
sum(!st_is_valid(radio_bsas))
...
```{r Visualizaciones 8: Selección debarios }
ggplot() + 
...
```{r Operaciones de datos4: Agrupación y filtrado radiocensales por barrio con toma de  muestra}
...
```{r Visualizaciones9: Muestra de 8 Radiocensales por barrio de clase media y alta seleccionado }
ggplot() + 
  geom_sf(data = radio_bsas, fill = "grey90", color = "black") +
...
```{r Operaciones espaciales4: Buffers y conteo de escuelas}
...
```{r Graficos4: Conteo de escuelas por radio de cobertura}
...
```{r Tabla2: Conteo de escuelas en barrios clase media y alta por tipo de gestión}
...
```{r Graficos5: Cantidad de escuelas en barrios de clase media y alta por gestión}
...
```{r  Visualizaciones 10: Escuelas por radio de cobertura en  radioscensales}
...
```{r  Visualizaciones12: Interactivadad}
leaflet() %>%
...
```{r Geconsulta3: preparación, message=FALSE, warning=FALSE}
...
```{r Graficos 6: Tiempo promedio por radiocensales}
...
```{r Graficos8: Escuelas por radio de cobertura y gurpo socio economico}
...
```{r Tabla5: Comparativa por tipo de gestion}
...
```{r Operaciones de datos5: Unificacion geoconsulta}
...
```{r Graficos10: Comparativa de tiempo a escuelas}
