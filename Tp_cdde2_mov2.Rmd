...
barrioscm_sjs <- st_read(url_barrioscaba) %>%
  st_make_valid() %>%
  st_transform(5347) %>%
  st_join(bairescm_q, join = st_intersects) %>% #Join espacial para ubicar radio censales en barrios
  select(CO_FRAC_RA, geometry, Remuneracion_media, cluster, #Seleccionamos columnas que nos interesan y renombramos
...
```{r Segunda Visualización: Buffers y escuelas}
#Visualizacion
ggplot() +
