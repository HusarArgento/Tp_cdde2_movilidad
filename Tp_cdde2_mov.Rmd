...
barrios_caba <- st_transform(barrios_caba, 4326)
barrios_q2 <- st_join(baires_q, barrios_caba, join = st_intersects)
barrios_q3 <- barrios_q2 %>% select(CO_FRAC_RA, geometry, Remuneracion_media, cluster, nombre, comuna, perimetro_, area_metro)
barrios_q3 %>% 
  filter(cluster == "Clase media") %>%
  filter(Remuneracion_media == max(Remuneracion_media, na.rm = TRUE))
...
ggplot() +
  ...
  geom_sf(data = baires_q2, aes(fill = Remuneracion_media)) +              # capa temática
  theme_minimal()+ 
