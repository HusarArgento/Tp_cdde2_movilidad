...
radios2 <- st_transform(radios2, 4326)
```
...
baires_ecorad <- mapa_eco  %>% filter(provincia_id == 2) #Filtrado de CABA
baires_q <- baires_ecorad %>% mutate(cluster = case_when(
  Remuneracion_media <= quantile(Remuneracion_media, 0.25)~ "Clase media baja", 
  Remuneracion_media <= quantile(Remuneracion_media, 0.50)~ "Clase media",
  Remuneracion_media <= quantile(Remuneracion_media, 0.75)~ "Clase media alta",
  Remuneracion_media > quantile(Remuneracion_media, 0.75)~ "Clase  alta"
))
#Clasificamos clusters
table(baires_q$cluster)
  
```
```{r}
#Cargamos un mapa de radiocensales de CABA para que nos oficie de base
radio_bsas <-  st_read(url_radiocensalcaba)
st_crs(radio_bsas)
sum(!st_is_valid(radio_bsas))
radio_bsas <- st_transform(radio_bsas, 4326)
```
...
ggplot() +
  geom_sf(data = radio_bsas, fill = "grey95", color = "black") +  # mapa base
  geom_sf(data = baires_q, aes(fill = cluster)) +              # capa temática
  theme_minimal()+ 
  labs(title = "Distribución de clases sociales sobre mapa base", fill = "Clase")
